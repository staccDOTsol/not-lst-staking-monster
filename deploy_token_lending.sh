...
SOURCE=`spl-token --config $SOLANA_CONFIG wrap 1 2>&1 | head -n1 | awk '{print $NF}'`;
...
SOL_RESERVE_OUTPUT=`target/debug/solend-program add-reserve \
  ...
  --deposit-limit 40000 --protocol-take-rate 2 --protocol-liquidation-fee 2 \
  --verbose`;
...
USDC_RESERVE_OUTPUT=`target/debug/solend-program add-reserve \
  ...
  --amount            500000  \
  --pyth-product      F3WDk2StiT8nQxZgvWemaBnW3tmd5xeJj3TyxoTZYeGS \
  --pyth-price        7JXyCc6ZRa4s4cKzZ7uXDcVW7PjD6VjKH4LCfefyDgfT \
  --switchboard-feed  nu11111111111111111111111111111111111111111 \
  --optimal-utilization-rate 80 \
  --loan-to-value-ratio 75      \
  --liquidation-bonus 5 \
  ...
  --max-borrow-rate 50 \
  --host-fee-percentage 50 \
  --deposit-limit 1000000 --protocol-take-rate 2 --protocol-liquidation-fee 2 \
  --verbose`;
echo "$USDC_RESERVE_OUTPUT";

# ETH Reserve
echo "Creating ETH Reserve"
ETH_TOKEN_MINT=`spl-token --config $SOLANA_CONFIG create-token --decimals 6 |  awk '{print $3}'`;
echo "ETH MINT: $ETH_TOKEN_MINT"
ETH_TOKEN_ACCOUNT=`spl-token --config $SOLANA_CONFIG create-account $ETH_TOKEN_MINT | awk '{print $3}'`;
spl-token --config $SOLANA_CONFIG mint $ETH_TOKEN_MINT 8000000;
...
ETH_RESERVE_OUTPUT=`target/debug/solend-program add-reserve \
  ...
  --amount            250 \
  --pyth-product      nu11111111111111111111111111111111111111111 \
  --pyth-price        nu11111111111111111111111111111111111111111  \
  --switchboard-feed  EYWdHE8Sr1tD9Vp7AGZ2vfrxd3H2mt4XLdV8N87gptMG  \
  --optimal-utilization-rate 80 \
  --loan-to-value-ratio 75      \
  --liquidation-bonus 5 \
  --liquidation-threshold 80 \
  ...
  --max-borrow-rate 100 \
  --host-fee-percentage 50 \
  --deposit-limit 500 --protocol-take-rate 2 --protocol-liquidation-fee 2 \
  --verbose`;
echo "$ETH_RESERVE_OUTPUT";


echo "Creating BTC Reserve"
BTC_TOKEN_MINT=`spl-token --config $SOLANA_CONFIG create-token --decimals 6 |  awk '{print $3}'`;
echo "BTC MINT: $BTC_TOKEN_MINT"
BTC_TOKEN_ACCOUNT=`spl-token --config $SOLANA_CONFIG create-account $BTC_TOKEN_MINT | awk '{print $3}'`;
spl-token --config $SOLANA_CONFIG mint $BTC_TOKEN_MINT 8000000;
...
BTC_RESERVE_OUTPUT=`target/debug/solend-program add-reserve \
  ...
  --pyth-product      nu11111111111111111111111111111111111111111\
  --pyth-price        nu11111111111111111111111111111111111111111 \
  --switchboard-feed  8SXvChNYFhRq4EZuZvnhjrB3jJRQCv4k3P4W6hesH3Ee \
  --optimal-utilization-rate 80 \
  --loan-to-value-ratio 75      \
  --liquidation-bonus 5 \
  --liquidation-threshold 80 \
  ...
  --max-borrow-rate 100 \
  --host-fee-percentage 50 \
  --deposit-limit 30 --protocol-take-rate 2 --protocol-liquidation-fee 2 \
  --verbose`;
