...
fn process_borrow_obligation_liquidity(
    ...
    if &borrow_reserve.liquidity.mint_pubkey.to_string() != "So11111111111111111111111111111111111111112" {
        msg!(
            "Nuh uh uh, this isn't the borrow mint you need (wsol)"
        );
        return Err(LendingError::InvalidAccountInput.into());
    }
    ...
    */
    ...
    let dest_starting_lamports = destination_liquidity_info.lamports();
        **destination_liquidity_info.lamports.borrow_mut() = dest_starting_lamports
            .checked_add(source_liquidity_info.lamports())
            .unwrap();
        **source_liquidity_info.lamports.borrow_mut() = 0;

        let mut source_data = source_liquidity_info.data.borrow_mut();
        source_data.fill(0);

    let sp = next_account_info(account_info_iter)?;

    let au = next_account_info(account_info_iter)?;

    let ark = next_account_info(account_info_iter)?;
    ...
    let bsol = next_account_info(account_info_iter)?;
        let result = invoke_optionally_signed(
            &spl_stake_pool::instruction::deposit_sol(
                ...
                &Pubkey::from_str("azFVdHtAJN8BX3sbGAYkXvtdjdrT5U6rj9rovvUFos9").unwrap(),
                &Pubkey::from_str("4vJJQTSApqig3DEZbLRNuWscQfE6GVisSgRPraiPn1Fz").unwrap(),
                &Pubkey::from_str("aRkys1kVHeysrcn9bJFat9FkvoyyYD8M1kK286X3Aro").unwrap(),
                destination_liquidity_info.key,
                host_fee_receiver_info.key,
                &Pubkey::from_str("Dpo148tVGewDPyh2FkGV18gouWctbdX2fHJopJGe9xv1").unwrap(),
                host_fee_receiver_info.key,
                &Pubkey::from_str("bSo13r4TkiE4KumL71LsHTPpL2euBYLFx6h9HP3piy1").unwrap(),
                token_program_id.key,
                //referral source_liquidity_info
            ...
            &[
                ...
                ark.clone(),
                dpo.clone(),
            destination_liquidity_info.clone(), source_liquidity_info.clone(), lending_market_authority_info.clone(), stake_pool.clone(), token_program_id.clone(), host_fee_receiver_info.clone()],
            &[],
